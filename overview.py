...
def load_state():
    ...
    except Exception: return pd.DataFrame(columns=["city","hrs","risk_level","value","lat","lon"])
@st.cache_data(ttl=300)
def load_raw():
    try:
        raw=pd.read_csv("raw_data.csv"); raw["timestamp"]=pd.to_datetime(raw["timestamp"]); return raw
    except Exception: return pd.DataFrame(columns=["city","location_name","parameter","value","timestamp","lat","lon"])
@st.cache_data(ttl=300)
def load_sensors():
