"""AirGuard NG — Main Dashboard (overview.py)"""
...
from styles import (
    BASE_CSS, RISK_COLORS, RISK_BG, RISK_BORDER, RISK_ADVICE,
    CONDITION_ADVICE, CITY_RENAME, section, badge, plotly_layout,
    calculate_hrs, load_device_data, classify_gas, gas_is_dangerous,
    device_status_bar,
)

st.set_page_config(page_title="AirGuard NG",page_icon="🛡️",layout="wide",initial_sidebar_state="expanded")
st.markdown(BASE_CSS, unsafe_allow_html=True)
st_autorefresh(interval=10000, key="overview_refresh")  # refresh every 10 seconds

def md(html): st.markdown(html, unsafe_allow_html=True)
def classify_gas_raw(raw):
    from styles import classify_gas
    return classify_gas(raw)
...
def load_state(): return pd.read_csv("transformed_data.csv")
@st.cache_data(ttl=300)
def load_raw():
    try:
...
@st.cache_data(ttl=300)
def load_sensors():
    if os.path.exists("sensor_locations.csv"): return pd.read_csv("sensor_locations.csv")
    return pd.DataFrame()
...
<p style="font-size:14px;color:#64748B;margin:0 0 24px;line-height:1.6">Set up your profile for personalised air quality insights and condition-aware health alerts across Nigeria.</p></div>""")
    ...
    df=load_state(); raw=load_raw(); sensor_locs=load_sensors()
    ...
    user_key=STATE_KEY.get(user_state,"Lagos State")
    ur=df[df["city"]==user_key]
    ...
    user_loc=(user_state or "Nigeria")+(f" · {user_city}" if user_city else "")
    cond_adv=CONDITION_ADVICE.get(user_condition,"")
    hero_sub=(cond_adv[:120]+"…" if len(cond_adv)>120 else cond_adv) if cond_adv else "Monitor real-time air quality across Nigeria and get health-aware guidance for your location."
  ...
  <p style="font-size:12px;color:#64748B;margin:0">Real-time Air Quality and Health Risk Intelligence — Nigeria</p></div>
  <div style="margin-left:auto;display:flex;align-items:center;gap:12px">
    ...
    # DEVICE STATUS BAR — always at top
    device_status_bar(st, location_label=user_state)

    # Gas danger banner
    if device:
        raw_gas=int(device.get("gas_raw",0) or 0)
        if gas_is_dangerous(raw_gas):
            ppm=device.get("gas_ppm",0)
            gl,gc,_,_,gi,grisk=classify_gas(raw_gas)
...
<p style="font-size:14px;color:#94A3B8;margin:0">Your indoor sensor is reading <strong style="color:#DC2626">{ppm} PPM</strong> — <strong style="color:#DC2626">{grisk}</strong>. Open all windows immediately. Do not use electrical switches. Check your gas cylinder. See the <a href="/Device" style="color:#F97316">Device page</a> for full details.</p>
</div>""")
...
<strong style="color:{worst_col};margin-left:6px">{worst_lbl} Air Quality Alert:</strong>
<span style="color:#94A3B8;margin-left:6px">{RISK_ADVICE.get(worst['risk_level'],'')}</span></div>""")
  ...
  <div style="flex:1;min-width:260px">
    <p style="font-size:12px;font-weight:600;color:#16A34A;text-transform:uppercase;letter-spacing:.08em;margin:0 0 6px">{greeting}, {fn}</p>
    <p style="font-family:Sora,sans-serif;font-size:24px;font-weight:700;color:#F8FAFC;letter-spacing:-.02em;line-height:1.25;margin:0 0 8px">Nigeria Air Quality Overview</p>
  ...
  <div style="background:rgba(22,163,74,0.10);border:1px solid rgba(22,163,74,0.28);border-radius:14px;padding:16px 22px;min-width:190px;flex-shrink:0;text-align:right">
    <p style="font-size:11px;font-weight:600;color:#16A34A;text-transform:uppercase;letter-spacing:.07em;margin:0 0 4px">Your Location Risk</p>
    ...
    <p style="font-size:12px;color:#64748B;margin:0 0 8px">{user_loc}</p>
    ...
    for col,(key,label,sensors_lbl,note) in zip([c1,c2,c3,c4],FIXED_CARDS):
        ...
        # Show device icon if user's state matches this card
        dev_badge=""
        if user_state and key.startswith(user_state.split()[0]):
            dev_badge='<span style="background:rgba(249,115,22,0.15);color:#F97316;border:1px solid rgba(249,115,22,0.3);padding:2px 8px;border-radius:999px;font-size:10px;font-weight:600;margin-left:6px">🔩 Device here</span>'
        with col:
            md(f"""<div style="background:{bg};border:1px solid {brd};border-radius:18px;padding:18px;position:relative;overflow:hidden;min-height:250px">
<div style="position:absolute;top:0;left:0;right:0;height:3px;background:{color}"></div>
<p style="font-family:Sora,sans-serif;font-size:14px;font-weight:700;color:#CBD5E1;margin:0 0 2px">{label}{dev_badge}</p>
...
<p style="font-size:10px;font-weight:600;color:#64748B;text-transform:uppercase;letter-spacing:.07em;margin:4px 0 10px">Health Risk Score</p>
...
<p style="font-size:12px;color:{color};margin:0 0 8px">{FULL.get(risk,risk)}</p>
<p style="font-size:11px;color:#4a5a7a;line-height:1.5;margin:0">{note}</p>
    ...
    with c5:
        ...
        ppm=device.get("gas_ppm","—") if device else "—"
        tmp=device.get("temperature","—") if device else "—"
        hmd=device.get("humidity","—") if device else "—"
        ts_d="—"
        if device:
        ...
        md(f"""<div style="background:linear-gradient(135deg,#111827 0%,rgba(249,115,22,0.06) 100%);border:1px solid {gbrd};border-radius:18px;padding:18px;position:relative;overflow:hidden;min-height:250px">
...
<span style="font-size:10px;font-weight:600;color:{sdot};text-transform:uppercase;letter-spacing:.06em">AirGuard Device · {slbl}</span>
</div>
<p style="font-family:JetBrains Mono,monospace;font-size:10px;color:#64748B;margin:0 0 6px">airguard-uno-01 · {user_state or 'Location not set'}</p>
...
<p style="font-family:Sora,sans-serif;font-size:2.8rem;font-weight:800;color:{gc};line-height:1;letter-spacing:-.04em;margin:10px 0 0">{ppm}</p>
<p style="font-size:10px;font-weight:600;color:#64748B;text-transform:uppercase;letter-spacing:.07em;margin:4px 0 10px">Gas PPM · MQ2 Sensor</p>
<div style="height:1px;background:rgba(255,255,255,0.06);margin-bottom:10px"></div>
<p style="font-size:12px;color:#64748B;margin:0 0 4px">🌡 <span style="color:#42a5f5">{tmp}°C</span> &nbsp; 💧 <span style="color:#26c6da">{hmd}%</span></p>
<p style="font-size:11px;color:#4a5a7a;margin:6px 0 0">Risk: <strong style="color:{gc}">{grisk}</strong></p>
<p style="font-family:JetBrains Mono,monospace;font-size:10px;color:#3a4a6a;margin:8px 0 0">Last: {ts_d}</p>
</div>""")
    ...
    with col_map:
        ...
        map_data=sensor_locs.copy() if not sensor_locs.empty else pd.DataFrame()
        if map_data.empty:
        ...
        m=folium.Map(location=[7.0,4.8],zoom_start=6,tiles="CartoDB dark_matter")
...
<span style='font-size:18px;font-weight:700;color:{color}'>HRS: {hv}</span><br>PM2.5: {pv} µg/m³<br>Temp: {tv} · Hum: {huv}<br><span style='color:{color}'>{rv}</span></div>"""
        ...
        if device:
            ...
            folium.Marker(location=[6.45,3.40],popup=folium.Popup(f"""<div style='font-family:sans-serif;background:#111827;color:#F8FAFC;border-radius:10px;padding:12px;min-width:180px'><b style='color:{gc2}'>🔩 AirGuard Device</b><br>{user_state or 'Lagos'}<br>Gas: {gp} ppm · {gl2}<br>Temp: {device.get('temperature','—')} °C<br>Hum: {device.get('humidity','—')} %</div>""",max_width=220),tooltip=f"AirGuard Device · {gp} ppm · {gl2}",icon=folium.Icon(color="orange",icon="star")).add_to(m)
        md(f'<p style="font-family:JetBrains Mono,monospace;font-size:11px;color:#64748B;margin:0 0 8px"><span class="ag-live"></span>{len(map_data)} sensors live</p>')
    ...
    with col_trend:
        ...
        options=[CITY_LBL[c] for c in available]; lbl2key=dict(zip(options,available))
        if not options:
            ...
            sel=st.selectbox("",options,label_visibility="collapsed")
            cdata=pm25_raw[pm25_raw["city"]==lbl2key[sel]].sort_values("timestamp")
            if cdata.empty:
    ...
    HCARDS=[
        ("⏰","rgba(249,115,22,0.12)","Best time to go outside","Based on today's pattern","The safest outdoor window in Lagos today is before 7:00 AM or after 7:00 PM when traffic-related PM2.5 and NO2 are at their lowest."),
        ("🫁","rgba(220,38,38,0.12)","Highest risk today",f"Alert: {worst_lbl}",f"{worst_lbl} has the highest HRS of {worst['hrs']} today. {RISK_ADVICE.get(worst['risk_level'],'')[:100]}"),
        ("💊","rgba(22,163,74,0.12)","Medication reminder","Pre-emptive dosing guidance","If you must go outside in a high-risk area today, take your prescribed medication 30 minutes before outdoor exposure rather than waiting for symptoms."),
        ("🏥","rgba(14,165,233,0.12)","Weekly exposure summary","For your next doctor visit",f"Average PM2.5: {avg_pm25} µg/m³ — approximately {round(avg_pm25/15,1)}x the WHO safe annual limit. Share this with your doctor."),
    ]
...
AirGuard NG · OpenAQ v3 · WHO 2021 · 3MTT NextGen · Environment Pillar · Nigeria 2026 · Updated {now_str} · Auto-refreshes every 10s</p></div>""")
